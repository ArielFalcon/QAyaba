...
/* Brand palette — truecolor. Ember is the single signal/accent (cursor, selection, the one boxed element). The verdict ramp — pass/fail/flaky/infra — is the ONLY source of status color; greys carry structure (rules) and hierarchy (dim labels, faint hints). lipgloss degrades these hexes on non-truecolor terminals, so authoring in truecolor is safe. */
var (
