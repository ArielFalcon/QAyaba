...
a backend, the convention services use to communicate by events, or the convention one backend uses
to call another over HTTP — and return candidate
...
- **File globs** (`frontFiles`, `files`, `sourceFiles`): FILENAME-SUFFIX globs only — `**/*.<suffix>` or `*.<suffix>`
...
- **`callPattern.kind`**: must be a supported extractor kind. Supported today:
  `"rest-template-exchange"` | `"feign-client"` | `"web-client"` — BE→BE REST clients
  (RestTemplate `.exchange` / `getForObject`, `@FeignClient` mappings, WebClient `.get().uri(...)`).
...
Propose this when backend repos call each other over HTTP (RestTemplate, Feign, WebClient). Frontend
HTTP clients stay `http` — do not reuse `http-backend` for FE→BE.

```json
{
  "transport": "http-backend",
  "sourceFiles": "<filename-suffix glob for backend client files, e.g. **/*.java>",
  "callPattern": { "kind": "rest-template-exchange", "receiver": "<optional field name, e.g. restTemplate>" },
  "servicePrefixTemplate": "<URL/client prefix template naming the target service, e.g. svc-{service}-api>",
  "serviceRepoTemplate": "<repo-name template the prefix maps to, e.g. ms-{service}>",
  "openApiPath": "<literal path to the service's OpenAPI file, e.g. src/main/resources/openapi/api-definition.yaml>"
}
...
`callPattern.kind` dialect: `"rest-template-exchange"` | `"feign-client"` | `"web-client"`.
