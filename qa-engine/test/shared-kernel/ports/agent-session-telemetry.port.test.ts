/* AgentSession.prompt must carry the per-call telemetry/contract-repair opts
   (textOnly/round/isRepair/sectionSizes), and AgentTurnEvent must surface the per-turn telemetry
   (round/isRepair/sectionSizes; runId nullable). tsc is the real assertion for the type widening;
   the runtime asserts the opts are forwarded verbatim (so a stub that silently dropped them would
   FAIL) and that a TurnTelemetrySink can record the enriched event.
 */
...
test("AgentSession.prompt forwards the per-call telemetry/repair opts verbatim", async () => {
  /* The session echoes the received opts so the test proves they reached prompt() unchanged — a
     session whose prompt ignored opts would not even compile here.
   */
...
test("AgentTurnEvent carries round/isRepair/sectionSizes and a TurnTelemetrySink records them", () => {
  ...
  /* runId is nullable (null for runs without a run context); sectionSizes is the
