...
test("selectBestProfile: two zero-link candidates (both unusable) → deterministic first-wins, not the noisier one", () => {
   ...
   */
  const sparse = withScore({ id: "sparse" }, makeScore(0, 2, 1, 2));
  const noisy = withScore({ id: "noisy" }, makeScore(0, 40, 30, 30));
...
test("selectBestProfile: nname-shape correct profile (links:10, ratio~0.37 -> resolvedScore~3.7) beats precise-but-sparse (links:2, ratio:1.0 -> resolvedScore 2.0) — positive-path check, not adversarial (both orderings agree)", () => {
  ...
  const sparse = withScore({ id: "sparse" }, makeScore(2, 0, 0, 0));
...
/* The tie-break must prefer PRECISION, not raw links. resolvedScore = links^2/coverage ties
   OFTEN on small integers (links:1,cov:1 == links:2,cov:4 == 1.0), and a `links` tie-break then
   picks the NOISIER candidate. This test is adversarial: perfect1 and noisy2of4 tie on
   resolvedScore (both 1.0) but disagree on both links (1 vs 2) and resolutionRatio (1.0 vs 0.5).
   A links-desc tie-break picks noisy2of4; a resolutionRatio-desc tie-break picks perfect1.
 */
...
/* A hand-built +Infinity resolvedScore must never win, order-independent.
   Number.isNaN alone leaves Infinity free to win the comparison. ANY non-finite value
   (NaN, +Infinity, -Infinity) must coerce to -Infinity via Number.isFinite.
 */
