...
/* assert a log line matched a regex — none of them asserts the ABSENCE of real I/O. An edit that
   ...
   The scan must catch a bare specifier (`from "child_process"`, no `node:` prefix — legal and
   common) and a dynamic import (`await import("node:child_process")`), not only `from "node:X"`.
   Evasion strings, not just against the (currently clean) real adapter source — a regression in
   the pattern set itself is caught even before it would ever show up as a real gap in the file.
 */
...
export const FORBIDDEN_IO_PATTERNS: RegExp[] = [
  ...
  /* CJS require, same bare/prefixed coverage — the same evasion class (a static-form scan that
     only recognizes one import shape).
   */
