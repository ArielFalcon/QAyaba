...
test("mergeAttrs: a RawAttr with no stable attributes (no testId/id/name/href) is not emitted", () => {
  const nodes = ["button: Submit"];
  const rawAttrs = [{ key: "button: Submit" }];
  const result = mergeAttrs(nodes, rawAttrs);
...
test("formatDomSnapshot keeps table/list roles past the node cap (a present table is never truncated away)", () => {
  ...
  const out = formatDomSnapshot([{ route: "/vets", nodes: [...nav, ...table] }]);
...
/* ── captureRouteTrees: per-route RAW node lines for the pre-execution selector check ────── */
...
test("buildChangedMarker: text fallback — node name contains changed.text → [CHANGED: added text …]", () => {
  const attr: NodeAttr | undefined = undefined;
...
/* capDomLines must NOT evict interactive nodes in favor of landmarks.
...
/* parseAriaSnapshot must NOT include a state suffix in nodes[] — the list stays bare.
 */
...
/* formatDomSnapshot renders the state suffix DISPLAY-ONLY (not in nodes[]). */
...
/* captureDomForRoutes is a fourth capture path; per-route degrade must be loud there too (a
   render that returns WITHOUT throwing but with errored routes was silently swallowed —
