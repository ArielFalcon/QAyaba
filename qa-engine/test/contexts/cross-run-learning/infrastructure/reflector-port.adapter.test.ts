...
/* the test above uses an llm-api-key-shaped secret, which
...
/* The use-case computes archetype from the run's diff (detectArchetype,
   cross-run-learning/domain/distill-rule.ts) and threads it onto ReflectionInput; the adapter must
   persist THAT value, never re-derive or re-fabricate its own. A saved rule must not leave
   archetype always null.
 */
...
/* A reviewer-rejection outcome's errorClass must be DETERMINISTICALLY derived from the real
   reviewerCorrections (correctionToErrorClass), never blindly trusted from the LLM's own echoed
   `reflection.errorClass` field. The prompt instructs the model "do NOT change it", but an LLM is
   non-deterministic — this channel is the authoritative source.
 */
...
/* The corrections-distillation channel is a new SIGNAL SOURCE (a more precisely classified
   errorClass + a real archetype tag), never a promotion bypass. A reviewer-rejection-derived rule
   must land EXACTLY where every other reflector-authored rule lands — candidate/low, zero
...
test("reflect() skips saving when the distilled rule normalizes onto an EXISTING rule's key, even a deprecated one", async () => {
  /* Dedup keys on the NORMALIZED trigger/action text; seed a deprecated rule sharing the
