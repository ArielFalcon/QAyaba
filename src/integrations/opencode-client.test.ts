...
test("buildPrompt sanitizes the diff (defense in depth)", () => {
  /* cappedDiffText now sanitizes the diff in "model" mode
...
test("buildWorkerPrompt injects the live a11y tree as GROUND TRUTH when provided (worker transcribes, not guesses)", () => {
  ...
  assert.doesNotMatch(buildWorkerPrompt(base), /GROUND TRUTH/);
...
/* The Lever-2 selector contradiction must be rendered in FULL, even when the case detail is long
