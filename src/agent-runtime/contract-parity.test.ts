/* Both AgentRuntimeStrategy implementations must expose the same observable contract, or an
   asymmetry must be listed in ALLOWED_ASYMMETRIES with a reason. Adding a capability to one
   strategy without the other fails this test.

   Known honest gaps:
   - onUsage: OpenCode emits token accounting; Codex exec JSONL does not — omitting onUsage
     rather than fabricating null snapshots.
...
/* Import the codex classifier to verify it is exported and callable. */
