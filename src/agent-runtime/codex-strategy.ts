...
export interface CodexTransportStartInput {
  ...
  signal?: AbortSignal;
  timeoutMs?: number;
  /* SIGTERM when the caller supplied timeoutMs; SIGKILL for the strategy's own default deadline. */
...
export class CodexRuntimeStrategy implements AgentRuntimeStrategy {
  ...
  async openSession(
    ...
    /* Caller timeoutMs wins; otherwise the per-role default so a Codex turn is never unbounded. */
...
/* True when the text contains a generator specs[]/approved block or a reviewer approved field. */
...
export function extractCodexLastMessage(jsonl: string): string {
  ...
  /* Prefer the last message that itself contains a verdict; otherwise the true last message. */
