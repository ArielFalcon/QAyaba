...
export interface QaCase {
   ...
   * capture missed. Never written to Issue bodies — only threaded into the reviewer prompt.
   */
  finalUrl?: string;
  /*
   * Deduped, capped (~15) browser-console `error` entries and uncaught `pageerror` exceptions
   * from the failing test (`text` truncated ~200 chars). Optional/best-effort, same absent-warned
   * contract as httpStatus/finalUrl. Diagnostic only — never blocks, auto-passes, or masks a
   * generated-test defect.
   */
...
/*
 * A structured, real-time activity event for the TUI. `kind` (and `status` for todos) lets the
 * dashboard categorize a live panel. Raw model-stream prose is never an activity.
 */
...
export interface RunOutcome {
  ...
  gateSignals: {
    ...
    /*
     * Pre-execution deterministic selector check (observation-only). `preExecAmbiguityCatches`:
     * strict-mode ambiguities against the live pre-write DOM; `deterministicSelectorBlocks`: how
     * many persisted after corrective regen and triggered the deterministic block. Never affects
     * the verdict on their own (the static gate does). Absent when the check did not run.
     */
    preExecAmbiguityCatches?: number;
    deterministicSelectorBlocks?: number;
    /*
     * Catalog gate (observation-only): test-id selectors gated in the confident window vs left
     * advisory, and fabricated test-ids caught pre-execution.
     */
