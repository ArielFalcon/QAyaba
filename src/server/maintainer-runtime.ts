...
export function createMaintainerRuntime(cfg: MaintainerConfig, fx: MaintainerSideEffects = defaultMaintainerSideEffects()): MaintainerRuntime {
  ...
  /* Clone qayaba into a persistent working copy (or fetch + reset an existing one to main). */
    ...
    try {
      ...
      try {
        ...
        const prompt = [
          "## Incident report",
          "",
          "The following incident(s) were detected in the qayaba system.",
          "Diagnose the root cause in the codebase (you are in the qayaba repo)",
        ...
        const pr = await fx.github.createPullRequest(cfg.selfRepo, {
          ...
          body: [
            ...
            ...(summary.justification
              ...
              : []),
            "",
            "⚠️ **Review required before merge.** This PR was auto-generated by the qayaba maintainer agent.",
          ].join("\n"),
        });
